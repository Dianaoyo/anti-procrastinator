...
from ai import respond_to_lazy, respond_to_tired
...
@user.callback_query(F.data=='lazy')
async def lazy(callback: CallbackQuery):
    user_id = callback.from_user.id
    text = await get_ai_responses(user_id, 'lazy')
    if text:
        await callback.message.answer(text)
    else:
        respond = await respond_to_lazy(user_id)
...
# async def remind_eat(message: types.Message, user_id):
#     text = text_in_lang(user_id, 'remind eat')
#     bot.send_message(user_id, text)
#
# @user.message(Command("remind"))
# async def set_reminder(message: types.Message, bot: Bot):
...
#         id=f"user_{message.from_user.id}"  # Уникальный ID заменяет старый будильник, если юзер переустановит время
#     )
#     await message.answer(f"Напоминание установлено на {hour:02d}:{minute:02d} каждый день.")